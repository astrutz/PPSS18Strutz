...
\begin{document}
	...
	\section{Einleitung} \label{Einleitung}
		...
		\begin{figure}[H] 
  				\centering
    			\includegraphics[height=0.3\textheight]{physicalBoardFar}
  				\caption{Beispiel eines physischen Boards im Unternehmen kernpunkt}
  				\label{fig:physKern}
		\end{figure}
		\noindent Das Board dient dem Team zur übersichtlichen Visualisierung des Sprints. Deshalb sollte es sich räumlich nah am Team befinden, bzw. gut vom Team erreichbar sein. Dem Kunden fehlt die Übersicht am Board jedoch, da dieser sich meistens nicht in direkter Nähe zum Team befindet. Daher wird die Darstellung der Stories oft durch Software wie z.B. ''JIRA'' von der Firma Atlassian umgesetzt. Auch hier lassen sich Stories in einem Sprint verplanen, mit Punkten schätzen und mit einem Status versehen.
		 \begin{figure}[H] 
  				\centering
    			\includegraphics[height=0.37\textheight]{jiraFar}
  				\caption{Darstellung einer Story in JIRA}
  				\label{fig:ANHjirasKernFar}
			\end{figure}
		 \noindent Da das Unternehmen ''kernpunkt'', in welchem diese Arbeit entwickelt wird, JIRA verwendet, fokussiert sich diese Arbeit auf JIRA als digitales Board und die zusätzliche Nutzung von physischen Boards
		\newpage
		\section{Grundlagen}
	...
	\end{itemize}
		...
		\subsubsection{Display mit Abbildung des JIRA}
		...
		\\ \\
		\textbf{Negativ: }Die Kosten eines Display sind deutlich höher als die eines Whiteboards oder einer Tafel in der gleichen Größe. Darüber hinaus kann das Display nach dem Kauf nicht vergrößert werden, während physische Boards mit zusätzlichen Boards erweitert werden können. Dies kann vor allem bei sich stetig verändernden Teams zu einem Problem werden. Die Nutzung eines Displays sorgt außerdem für eine geringere Aufmerksamkeit seitens des Teams \cite{pen}. Die fehlende Haptik senkt die Motivation des Teams \cite{progress}. 
		\subsubsection{Smartboards mit Abbildung des JIRA}
...
Die dritte Kernfunktionalität ist die Verbindung mit dem Server. Die Karte muss in der Lage sein sowohl Sensorendaten zu versenden als auch Anweisungen an das Display zu empfangen. Diese Daten werden mit dem Server per HTTP als JSON verschickt und auch als solche empfangen. Näheres zur Verbindung wird in Kapitel \ref{techserver} beschrieben.  
		...
		\subsection{Skizzierung der Systemarchitektur}
		In Abbildung \ref{fig:Architekturskizze} wird die verteilte Systemarchitektur erläutert. Hierbei wird das Anwendungsschichtprotokoll HTTP mit der Datenstruktur JSON genutzt. Als Schnittstelle wird hierbei das Konzept REST verwendet.  \\
		\begin{figure}[htbp] 
  			\centering
		...
		\end{figure}
	\newpage	
	\section{Abwägung und Entscheidung zwischen erhältlichen Technologien}
...
Daher eignen sich e-Ink Displays sehr gut für eine längere Nutzung, insbesondere auch für die Nutzung im Deep Sleep Mode, was die Akkulaufzeit einer Karte positiv beeinflusst. 
		...
		\subsection{Veränderungen des Status mittels Sensorik} \label{changeStatus}
		Um den Status einer Story zu erfassen, soll es möglich sein diesen an einer Karte zu verändern. Wichtig hierbei sind Minimierung möglicher Fehler, eine einfache Wartung und Portierbarkeit. Auch sollte die Lösung keine weitere Stromversorgung seitens der Karte benötigen.
			\subsubsection{Bestimmung der Position durch RFID-Tags}
			...
			\subsubsection{Ausstatten der Karten mit Vibrationssensor}
			...
			\subsubsection{Bestimmung der Position durch Infrarot}
			...
			\subsubsection{Bestimmung der Position durch Bluetooth Beacons}
			...
			\subsubsection{Schalter an der Karte zur manuellen Statusveränderung}
...
Im Prototyp werden die Karten mit Schaltern ausgestattet. Das stellt jedoch keine dauerhafte Lösung dar. Wenn die Nutzer in der Evaluation Probleme mit der Benutzung bekommen, werden hier weitere Technologien und Ansätze in Erwägung gezogen. Die vierte Story ''Status von Stories und Unteraufgaben verändern'' lässt sich damit jedoch umsetzen.
		\subsection{Server als REST-API zur Kommunikation mit JIRA und Karte} \label{techserver}
		...
		\subsection{Auswahl der JIRA API} \label{decideJira}
		...
		\subsection{Stromversorgung der Karte}
	...
	Im folgenden Kapitel wird die Umsetzung der User Stories in Form eines Prototypen erläutert. Hierbei liegt der Fokus auf der Anwendungslogik und den Anschluss von Sensoren an die jeweiligen Mikrocontrollern.
		\subsection{Umsetzung eines Anmeldeterminals zur Verwendung des Boards in Dailies}
			...
			Wie in Kapitel \ref{esp8266} beschrieben wird die Anmeldung mit RFID-Tags und einem RC522 Leser an einem ESP8266 mit NodeMCU umgesetzt. Der RFID-Reader kommuniziert über das Serial Peripheral Interface, kurz SPI mit dem Mikrocontroller. Das SPI wird für eine synchron-serielle Verbindung zwischen einem Host (in dem Fall der Mikrocontroller) und einem Peripheriegerät (in dem Fall der Reader) verwendet \cite{spistandard}. Hierfür sind vier SPI-spezifische, sowie drei Mikrocontroller-spezifische Leitungen notwendig. \\ \\
		...
		\begin{figure}[H] 
  			\centering
    		\includegraphics[height=0.4\textheight]{terminalHead}
		...
		Ebenfalls initiiert wird die serielle Verbindung zwischen dem Entwicklungsrechner und dem Mikrocontroller auf einer Rate von 115200 Baud. Sie ist für Logging und Fehlermeldungen nötig. So lässt sich am Rechner der USB-Port, an dem der Controller angeschlossen ist auf der Baud-Rate auslesen. Diese Verbindung wird mit Serial.begin() gestartet und mit Befehlen wie Serial.print(''Text'') angesprochen. Zur Weiterleitung der Terminalanmeldungen an den Server wird eine WLAN-Verbindung mit WiFi.begin(ssid, passwort) gestartet und auf eine erfolgreiche Verbindung (Status ''WL\_CONNECTED'') gewartet. Während des Herstellens der Verbindung werden keine weiteren Programmteile ausgeführt. Nach erfolgreicher Verbindung werden das SPI und der RFID-Reader mit Methoden der jeweiligen Bibliotheken initiiert. Zum Schluss wird über die serielle Verbindung eine Meldung über das abgeschlossene Setup ausgegeben. \\
			\begin{figure}[H] 
  			\centering
    		\includegraphics[height=0.35\textheight]{terminalSetup}
		...
		\begin{figure}[H] 
  			\centering
    		\includegraphics[height=0.55\textheight]{storyHead}
  			\caption{Terminalcode mit Instanzierung von globalen Variablen. Hinweis: SSID und Passwort des Drahtlosnetzwerkes wurden aus datenschutzrechtlichen Gründen entfernt.}
  			\label{fig:storyHead}
		\end{figure}
		...
		\subsection{Umsetzung eines NodeJS-Servers als Verbindung zwischen Board und JIRA} \label{implServer}
			...
			\begin{figure}[H] 
  				\centering
    			\includegraphics[height=0.45\textheight]{serverFilterData}
			...
			\begin{figure}[H] 
  				\centering
    			\includegraphics[height=0.35\textheight]{serverLoginLogoff}
			...
			\begin{figure}[H] 
  				\centering
    			\includegraphics[height=0.15\textheight]{serverStartDaily}
 	...
 	\begin{appendix}
 		\section{JSON-Strukturen} \label{ANHJSON}
 	 	...
 	 	\section{Ergänzungen zur Server-Implementation} \label{ANHServer}
 	  		\begin{figure}[H] 
  				\centering
    			\includegraphics[height=0.18\textheight]{serverHead}
  				\caption{Servercode der globalen Variablen und Instanzierungen}
  				\label{fig:ANHserverHead}
			\end{figure}
 	  		\begin{figure}[H] 
  				\centering
    			\includegraphics[height=0.075\textheight]{serverAppListen}
  				\caption{Servercode der Initialisierung}
  				\label{fig:ANHserverAppListen}
			\end{figure}
 	  		\begin{figure}[H] 
  				\centering
    			\includegraphics[height=0.3\textheight]{serverUpdateLocal}
  				\caption{Servercode der Aktualisierung der lokal gespeicherten Stories}
  				\label{fig:ANHserverUpdateLocal}
			\end{figure}
 	  		\begin{figure}[H] 
  				\centering
    			\includegraphics[height=0.55\textheight]{serverHelper}
		...
		\newpage 	  	
 	  	\section{Physisches Board} \label{ANHPhysischesBoard}
 	  		\begin{figure}[H] 
    			...
    			\includegraphics[height=0.3\textheight]{physicalBoardFar}
  				\caption{Beispiel eines physischen Boards im Unternehmen kernpunkt}
  				\label{fig:ANHphysKern}
			\end{figure}
			\begin{figure}[H] 
  				\centering
    			\includegraphics[height=0.3\textheight]{physicalBoardNear}
  				\caption{Spalten des physischen Boards}
  				\label{fig:ANHphysKernNear}
			\end{figure}
		\newpage 	  	
 	  	\section{Digitales Board} \label{ANHJIRA}
 	  		\begin{figure}[H] 
  				\centering
    			\includegraphics[height=0.37\textheight]{jiraFar}
  				\caption{Darstellung einer Story in JIRA}
			...
			\begin{figure}[H] 
  				\centering
    			\includegraphics[height=0.18\textheight]{jiraAssignee}
  				\caption{Darstellung eines Bearbeiters in JIRA}
  				\label{fig:ANHjirasKernAsign}
			\end{figure}
			\begin{figure}[H] 
  				\centering
    			\includegraphics[height=0.15\textheight]{jiraSubtasks}
  				\caption{Darstellung von Unteraufgaben in JIRA}
  				\label{fig:ANHjirasKernSub}
			\end{figure}
 	  	\newpage 	  	
 	  	\section{Prototyp} \label{ANHPrototyp}
 	  		...
 	  		\begin{figure}[H] 
  				\centering
    			\includegraphics[height=0.4\textheight]{cardFar}
  				\caption{Prototyp zur Darstellung von Stories und Unteraufgaben}
  				\label{fig:ANHcardFar}
			\end{figure}
			\begin{figure}[H] 
  				\centering
    			\includegraphics[height=0.4\textheight]{cardNear}
  				\caption{Nahaufnahme mit Blick auf die verwendeten GPIO-Pins}
  				\label{fig:ANHcardNear}
			\end{figure}
			\subsection{Terminal}
			\begin{figure}[H] 
  				\centering
    			\includegraphics[height=0.4\textheight]{terminalFar}
  				\caption{Prototyp zur Anmeldung von Teammitgliedern}
  				\label{fig:ANHterminalFar}
			\end{figure}
			\begin{figure}[H] 
  				\centering
    			\includegraphics[height=0.4\textheight]{terminalNear}
  				\caption{Nahaufnahme mit Blick auf die verwendeten GPIO-Pins}
  				\label{fig:ANHterminalNear}
			\end{figure}
 	\end{appendix}
