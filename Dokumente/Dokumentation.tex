...
Im Prototyp werden die Karten mit Schalter ausgestattet. Das stellt jedoch keine dauerhafte Lösung dar. Wenn die Nutzer in der Evaluation (in der Bachelorarbeit) Probleme mit der Benutzung bekommen, werden hier weitere Technologien und Ansätze in Erwägung gezogen. Der vierte PoC lässt sich damit jedoch umsetzen.
	...
	\newpage	
	\section{Fazit}
		\subsection{Prozessassessment}
			\subsubsection{Kooperation mit dem Unternehmen}
			\subsubsection{Einhaltung des Zeitplans}
			\subsubsection{Entwicklung des Systems}
			\subsubsection{Anfertigung der Dokumentation}
		\subsection{Diskussion des Zielerreichungsgrades}
			\subsubsection{Produktivitätssteigerung}
			\subsubsection{Vermeidung von Mehrkosten}
			\subsubsection{Vermeidung von Inkonsistenzen}
		\subsection{Ausblick}
	\newpage
